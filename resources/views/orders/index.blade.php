                                        ...
                                        <table class="table">
                                            ...
                                            @foreach ($order->items as $index=>$item)
                                                <tr>
                                                    ...
                                                    <td class="sku-amount text-conter">{{$item->amount}}</td>
                                                    @if ($index===0)
                                                        ...
                                                        <td  rowspan="{{count($order->items)}}" class="text-center">
                                                            ...
                                                            @else
                                                                未支付<br>
                                                                请在{{$order->created_at->addSeconds(config('app.order_ttl'))->format('H:i')}}前完成支付<br>
                                                                否则订单将自动关闭
                                                            @endif
                                                        </td>
                                                        <td rowspan="{{count($order->items)}}" class="text-center">
                                                            <a href="" class="btn-primary btn-sm btn">查看订单</a>
                                                        </td>
                                                    @endif
                                                </tr>
                                            @endforeach
